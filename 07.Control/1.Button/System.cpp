...
bool SystemClass::InitializeWindows()
{
	...
	// 프로그램 이름 설정.
	m_applicationName = TEXT("MyButton");
...
LRESULT SystemClass::MessageHandler(HWND hWnd, UINT iMessage, WPARAM wParam, LPARAM lParam)
{
	...
	switch (iMessage) {
	case WM_CREATE:
		CreateWindow(TEXT("button"), TEXT("Click Me"), WS_CHILD | WS_VISIBLE | BS_PUSHBUTTON, 20, 20, 100, 25, hWnd, (HMENU)0, m_hInstance, NULL);
		CreateWindow(TEXT("button"), TEXT("Me to"), WS_CHILD | WS_VISIBLE | BS_PUSHBUTTON, 20, 50, 100, 25, hWnd, (HMENU)1, m_hInstance, NULL);
		return 0;
	case WM_COMMAND:
		...
		case 0:
			MessageBeep(0);
			MessageBox(hWnd, TEXT("First Button Click"), TEXT("Click Me"), MB_OK);
			break;
		case 1:
			MessageBeep(2);
			MessageBox(hWnd, TEXT("Me to Click"), TEXT("Me to"), MB_OK);
			break;
		}
	case WM_KEYDOWN:
		return 0;
